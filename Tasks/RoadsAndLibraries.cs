...
namespace Tasks
{
    ...
    public class RoadsAndLibraries
    {
        ...
        {
			int city1number = city1 - 1;
			int city2number = city2 - 1;
            ...
            else if (_graph[city1number] != _graph[city2number])
            {
                ...
                foreach (int city in _graph[city2number]) {
        ...
        private long CalcMinCost()
        {
			...
			int unconnectedCities = _graph.Where(x => x == null).Count();
			long money = unconnectedCities * CostToBuildLibrary;

			long[] arr = _graph.Where(x => x != null).Distinct().Select(x => (long)x.Count).ToArray();
